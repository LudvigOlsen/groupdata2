...
run_balance_ <- function(data,
                         ...
                         mark_new_rows,
                         new_rows_col_name) {
  check_balance_always(data = data,
                       cat_col = cat_col,
                       id_col = id_col)
  ...
  if (!isTRUE(mark_new_rows)) {
    ...
    # e.g. '.new_row'
    balanced <- base_rename(data = balanced,
...
check_balance_once <- function(data,
                               size,
                               cat_col,
                               id_col,
                               id_method,
  ...
  if (!(
    (
      checkmate::test_string(size) &&
      size %in% c("min", "max", "mean", "median")
    ) ||
  ...
  checkmate::reportAssertions(assert_collection)
  if (!is.null(cat_col) && !is.null(id_col)) {
...
check_balance_always <- function(data, cat_col, id_col) {
  # Check arguments ####
  assert_collection <- checkmate::makeAssertCollection()
  checkmate::assert_data_frame(x = data,
                               min.rows = 1,
