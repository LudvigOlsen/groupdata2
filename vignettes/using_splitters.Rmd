...
output: 
  ...
  rmarkdown::pdf_document:
    ...
    toc_depth: 4
vignette: >
  %\VignetteIndexEntry{Using splitters}
...
In the examples we will be using knitr::kable() to visualize some of the data such as dataframes. You do not need to use kable() in any way when using the splitter functions.  
...
## Method: 'greedy'
'greedy' uses group **size** for splitting the data.  
...
'n_dist' uses a specified number of groups to split the data.  
...
## Method: 'n_fill'
'n_fill' uses a specified number of groups to split the data.  
First it creates equal groups as large as possible. Then, if there
...
## Method: 'n_last'
'n_last' uses a specified number of groups to split the data.   

With *default settings*, it tries to make the groups as equal as possible,
...
contain a lot of elements. Asked to split a vector with 57 elements into 20 groups, 
the first 19 groups will contain 2 elements, while the last group will itself contain 19 elements. 
Had we instead asked it to split the vector into 19 groups, we would have had 3 elements 
in all groups.
...
E.g. the participant must always have the same diagnosis (a or b) throughout the dataset. Else, the participant might be placed in multiple folds.  
...
df_folded <- df_folded[order(df_folded$.groups),]
...
Let's count how many of each diagnoses are in each group.  

```{r}

df_folded %>% group_by(.groups) %>% count(diagnosis) %>% kable(align='c')
...
df_folded %>% group_by(.groups) %>% count(participant) %>% kable(align='c')
...
Let's count how many of each diagnoses are in each group and find which participants are in which groups.

```{r}

df_folded %>% group_by(.groups) %>% count(diagnosis, participant) %>% kable(align='c')
...
### Vector with 57 elements split in 6 groups

Below you'll see a dataframe with counts of group elements when splitting the same data with the different n_ methods. 
...
### Vector with 117 elements split in 11 groups
...
Below we will take a short look at the cumulative sum of group elements to get an idea of what is going on under the hood.  
