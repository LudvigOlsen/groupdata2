...
date: "`r Sys.Date()`"
abstract: |
  groupdata2 is a set of subsetting methods for easy grouping, windowing, folding, splitting and balancing of data.  
  Create balanced folds for cross-validation or divide a time series into windows.  
  Balance sample sizes with up- and downsampling.  
...
# General information
groupdata2 is a set of functions and methods for easy grouping, windowing, folding, splitting and balancing of data.  
...
### balance()
Uses up- or downsampling to fix the group size to the min, max, mean, or median group size or to a specific number of rows. 
...
There are currently 3 methods for balancing on ID level in balance().  
...
partition() first divides up the dataframe by cat_col and then create n partitions for both diagnoses.  
