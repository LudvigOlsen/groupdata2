...
groupdata2 is a set of tools for creating groups from your data. It consists of five, easy to use, main functions, namely **group_factor**(), **group**(), **splt**(), **partition**(), and **fold**().  
...
**partition**() creates (optionally) balanced partitions (e.g. train/test sets) from given group sizes. It can balance partitions on one categorical variable (e.g. diagnosis) and/or is able to keep all datapoints with a shared ID (e.g. participant) in the same partition.
