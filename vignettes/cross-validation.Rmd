...
In this vignette we will train a couple of linear regression models to some data. We will use cross-validation with balanced folds to find the best model, and then test that model on a subset of the original data that we haven't used for training the model. 
...
Now our test set contains 2 participants (8 and 10), and the diagnosis column now contains half 'a' and half 'b'. Let's count it for the training set instead of looking at it.
...
In this section we will create balanced folds for cross-validation. The thoughts behind doing so will resemble those of the previous section. **fold**() basically just creates a number of ~equally sized partitions. Where **partition**() returned a list (this is optional btw.), **fold**() will return the entire training set but with a new column called ".folds". This will be used directly in the cross-validation function to subset the data on each iteration - remember, the folds take shifts being the test set (not to be confused with the one we just created before).
...
Fold 1 contains particants 2 and 7 with 3 rows each. Participant 2 has the diagnosis 'b' and participant 7 is an 'a'. This pattern is the same for all folds. Of course our data was created for the purpose of showing these functions, so real world data is likely less perfect, which is why you always want to investigate the output to have a sense of what is going on. Also beware that the max number of folds are restricted by the number of participants in the classes, if using the id_col and the cat_col arguments together. Remember, first the function subsets the dataset by cat_col, then it creates groups (folds) from the unique values in id_col in each subset and merges the subsets. So if you only have 3 unique participants in one class, this will be the max number of folds you can create.

# Cross-validation

What's left, you ask? Well, now we get to have fun, training and comparing models using cross-validation! If you just came for the partition and fold functions, you can skip this and.. well start using the functions in your own code instead. But I will move on and walk you through a simple cross-validation function for testing some models for predicting the scores of the participants.
...
That was it. If you have questions or ideas for improvement, send me a mail at r-pkgs@ludvigolsen.dk or open an issue on the github page https://github.com/LudvigOlsen/groupdata2 

Check out the full description vignette for groupdata2 [here](description_of_groupdata2.html)
